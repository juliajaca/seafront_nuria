...
posidonia600 = np.stack((
                     np.loadtxt(path+'POSIDONIA_600_0.5.txt'),
                    np.loadtxt(path+'POSIDONIA_600_2.txt'),
                    np.loadtxt(path+'POSIDONIA_600_5.txt'),
                     np.loadtxt(path+'POSIDONIA_600_7.txt')))

posidonia0 = np.loadtxt(path+'POSIDONIA_0_9999.txt')
...
# %%
play = pd.read_csv('C:/Users/Julia/Documents/VSCODE/src/carpeta_ignorada/9_paper/med_con_ss_con_ru.csv', sep=';')
...
for i in range(0,1):
    i=1
    print(i)
    if play.iloc[i]['actual'] < play.iloc[i]['actual_0']: 
        ...
        # SEGUNDO PASO
        #  la densidad inicial
        densidades =  [674 + 24.4 * prof_planta ]
        temperaturas = [devolver_sst2(play.iloc[i]['lat'],play.iloc[i]['lon'] , 2025)]

        for año in range(2026, 2100):
            sst = devolver_sst2(play.iloc[i]['lat'],play.iloc[i]['lon'] , año)
            # print(f'la lat lon son {play.iloc[i]["lat"]}, {play.iloc[i]["lon"]}')
            # valor_modelo = df_sst.loc[df_sst.iloc[:, 0] == año]
            # sst = float(valor_modelo.iloc[0, 1])  # Primera fila, segunda columna
            # print(sst)
            temperaturas.append(sst)
        ...
        print(temperaturas)
        print(densidades)
        años = list(range(2025, 2100))

        fila_playa = {
            "nombre": play.iloc[i]['nombre'],
            "provincia": play.iloc[i]['provincia'],
        }

        for año, densidad in zip(años,densidades):
            ...
            # print('-----')
            fila_playa[str(año)] = ru   

        todas_playas.append(fila_playa)

        plt.figure(figsize=(10, 5))
        plt.plot(range(2025, 2100), 
                 [fila_playa[str(año)] for año in años], 
                 marker='o', linestyle='-', color='teal')
        plt.title(f"Serie temporal de RU (2025–2099) para playa de la {play.iloc[i]['nombre']} en {play.iloc[i]['provincia']} con densidad inicial {densidades[0]}")
        plt.scatter(2025, play.iloc[i]['actual'] , color='red', label ='ru 2025 matrices 370')
        # plt.scatter(2025, 669 , color='green', label ='densidad inicial')
        plt.legend()
        plt.xlabel("Año")
        plt.ylabel("Valor RU")
        plt.grid(True)
        ...
        plt.figure(figsize=(10, 5))
        plt.plot(range(2025, 2100), temperaturas, marker='o', linestyle='-', color='red')
        plt.title(f"Serie temporal de tª (2025–2099) para playa de la {play.iloc[i]['nombre']} en {play.iloc[i]['provincia']} con densidad inicial {densidades[0]}")
        plt.legend()
        plt.xlabel("Año ")
        plt.ylabel("Valor Tª")
        plt.grid(True)
        ...
        no_posi = no_posi +1
        print('No HAY POSIDONIA!!!'   )   

df = pd.DataFrame(todas_playas)

# df.to_csv("salida4_con_temp_mejorada.csv", index=False) #salen 591 habra que hacerlo otra vez
# %%
print(df.head)
# %%
# Calculo incremento RU
# %%
años_objetivo = [2050, 2075, 2099]
año_inicio = 2025

for año in años_objetivo:
    dif = df[str(año)] - df[str(año_inicio)]  # incremento RU
    media = dif.mean()
    std = dif.std()
    stderr = dif.sem()  # standard error of the mean
    
    print(f"Año {año}:")
    print(f"  Incremento medio: {media:.4f} m")
    print(f"  ± Desviación estándar: {std:.4f} m")
    print(f"  ± Error estándar: {stderr:.4f} m\n")
# %%
