...
for fila in range(len(posi_cata[0:])):
    # fila = 28
    parche = posi_cata.iloc[fila].geometry
   ...
   # PUNTOS CADA 10 METROS
    ...
    df = pd.DataFrame({"geometry": points,
                    ...
                    'parche': contador,
                    'm2_punto':posi_cata.iloc[fila].Area_ha/len(points)*10000,
                    'm2_total': posi_cata.iloc[fila].Area_ha*10000,
                    'zona':'cataluña'})
    ...
    # fig, ax = plt.subplots(figsize=(6, 6))  # Aumenta el tamaño para mejor visualización
    # # Plot del GeoDataFrame principal
    # gdf.plot(column='profundidad', legend=True, ax=ax, cmap='viridis')
    # # Plot del parche en el mismo eje
    # gpd.GeoDataFrame(geometry=[parche], crs=gdf.crs).plot(ax=ax, color="red", linewidth=2, markersize=100)
    # # Personalización
    # ax.scatter([p.x for p in grid_points], [p.y for p in grid_points], color="blue", marker="o", s=20, label="Puntos")
